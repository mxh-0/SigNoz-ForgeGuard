# n8nForge Observer — Project Documentation
...
n8nForge Observer is a **self-healing multi-agent system** with an SRE Copilot supervisor that catches bad output before the developer ever sees it.
...
| Traditional Agent System | n8nForge Observer |
|---|---|
...
- `n8nforge.copilot.anomalies_detected` — broken down by anomaly type and agent step
- `n8nforge.copilot.healing_attempts` — by strategy (reword/rethink) and step
- `n8nforge.copilot.healing_successes` — how often fixes actually worked
- `n8nforge.copilot.manual_mode_triggers` — escalation rate
- `n8nforge.llm.latency_ms` — histogram of LLM response times
- `n8nforge.llm.errors_total` — by type: timeout, rate_limit, http_error
- `n8nforge.tasks.submitted/completed/failed` — the task funnel
...
The project name "n8nForge" reflects the workflow orchestration philosophy inspired by n8n:
...
2. Navigate to Services → `n8nforge-observer`
...
5. Check Metrics Explorer: see `n8nforge_copilot_anomalies_detected_total` increasing
